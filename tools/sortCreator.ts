...
Promise.all(
  fileNamesParts.map(fileName => {
    try {
      ...
      defaultJson.category = 'description' in originalJson.info
        ? keywords.filter(str => originalJson.info.description.includes(str))||[]
        : []
  ...
  const counter = descriptions.join(' ').toLowerCase().split(',').join(' ').split(' ').reduce(
    (counter: Record<string, number>, str: string) => {
  ...
  fs.writeFile(`descriptions.json`, JSON.stringify(descriptions), (error) => {})
  fs.writeFile(`wordCounter.json`, JSON.stringify(wordCounter), (error) => {})
